...
import {
  ...
  prepPayForDeveloperTxn,
} from 'metaplex/candy-machine/actions/instructions';
import { sendTransactionWithRetry } from 'metaplex/candy-machine/actions/transactions';
import { RemainingAccount } from 'metaplex/candy-machine/types';
...
export async function mint(
  ...
  const remainingAccounts: Array<RemainingAccount> = [];
  const signers = [mint];
  if (keypair instanceof Keypair) {
    signers.push(userKeyPair);
  }
  const { instructions: pushInstructions } = await prepPayForDeveloperTxn(
    userKeyPair.publicKey,
    extraMintPrice,
  );
  ...
  cacheContent.metadataSecureStore = encryptString(mint);
