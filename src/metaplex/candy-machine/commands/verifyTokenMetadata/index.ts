...
import { EXTENSION_JSON, EXTENSION_PNG } from 'metaplex/common/constants';
...
export const verifyAssets = ({ files, uploadElementsCount }) => {
  ...
  logger.info(`Verifying token metadata for ${pngFileCount} (png+json) pairs`);
};
...
export const verifyImageURL = (image, files, manifestFile) => {
  ...
  if (image !== expectedImagePath) {
    ...
    logger.warn(`We expected the \`image\` property in ${manifestFile} to be ${expectedImagePath}.
