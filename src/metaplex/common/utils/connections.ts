...
import { sleep } from 'metaplex/common/utils/utils';
  ...
  for (let i = 0; i < signedTxns.length; i++) {
    ...
    if (sequenceType !== SequenceType.Parallel) {
      try {
        ...
        logger.log('Caught failure', e);
        if (breakEarlyObject.breakEarly) {
          logger.log('Died on ', breakEarlyObject.i);
...
export async function sendSignedTransaction({
  ...
  logger.log('Latency', txid, getUnixTs() - startTime);
  return { txid, slot };
}
    ...
    while (!done && queryStatus) {
      ...
      (async () => {
        try {
          ...
          if (!done) {
            if (!status) {
              logger.log('REST result for', txid, status);
            } else if (status.err) {
              logger.log('REST error for', txid, status);
              done = true;
              reject(status.err);
